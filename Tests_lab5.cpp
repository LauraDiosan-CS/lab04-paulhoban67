...
void test_service_accounts()
{
	...
	strcpy_s(type_1, strlen("In") + 1, "In");
	strcpy_s(type_2, strlen("Out") + 1, "Out");
	strcpy_s(description_1, strlen("Mancare") + 1, "Mancare");
	strcpy_s(description_2, strlen("Salar") + 1, "Salar");
	Accounts a1(5, 65, type_1, description_1);
	Accounts a2(12, 13, type_2, description_2);
	Service serv_accounts;
	serv_accounts.add(a1);
	serv_accounts.add(a2);
	assert(serv_accounts.get_size() == 2);
	assert(serv_accounts.show()[0] == a1);
	assert(serv_accounts.show()[1] == a2);
	serv_accounts.del(a2);
	assert(serv_accounts.get_size() == 1);
